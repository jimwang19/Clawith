...
async def report_result(
    ...
    # Fallback: no active WS and this is a non-progress result.
    # For DingTalk-originated sessions, send to the sender's DingTalk account
    # to avoid silent updates when the frontend is offline.
    if body.result and msg.conversation_id and msg.sender_user_id and not ws_delivered:
        result_text = (body.result or "").strip()
        is_progress = result_text.startswith("⏳")
        if not is_progress and result_text:
            try:
                ...
                if session_obj and getattr(session_obj, "source_channel", None) == "dingtalk":
                    user_r = await db.execute(select(User).where(User.id == msg.sender_user_id))
                    sender_user = user_r.scalar_one_or_none()
                    username = getattr(sender_user, "username", "") or ""

                    if username.startswith("dingtalk_"):
                        dingtalk_user_id = username[len("dingtalk_"):]
                        ...
                        if dt_config:
                            # NOTE: current dingtalk_service only supports OTO/corp notification.
                            # For group sessions we still fallback to OTO to guarantee delivery.
                            if getattr(session_obj, "is_group", False):
                                logger.warning(
                                    f"[Gateway] DingTalk group session fallback via OTO for conv={msg.conversation_id}"
                                )
