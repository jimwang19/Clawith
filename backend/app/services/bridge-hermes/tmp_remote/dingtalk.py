...
# external_conv_id -> (session_webhook, ts)
_SESSION_WEBHOOK_CACHE: dict[str, tuple[str, float]] = {}
_SESSION_WEBHOOK_TTL = 3600


def cache_session_webhook(external_conv_id: str, session_webhook: str):
    if not external_conv_id or not session_webhook:
        return
    _SESSION_WEBHOOK_CACHE[external_conv_id] = (session_webhook, time.time())


def get_cached_session_webhook(external_conv_id: str) -> str | None:
    item = _SESSION_WEBHOOK_CACHE.get(external_conv_id)
    if not item:
        return None
    webhook, ts = item
    if time.time() - ts > _SESSION_WEBHOOK_TTL:
        _SESSION_WEBHOOK_CACHE.pop(external_conv_id, None)
        return None
    return webhook
...
async def process_dingtalk_message(
    ...
    async with async_session() as db:
        ...
        # Determine conv_id and group context for session isolation
        ...
        logger.info(
            f"[DingTalk] inbound conv_type={conversation_type} "
            f"is_group={is_group_chat} conv_id={conv_id}"
        )
        cache_session_webhook(conv_id, session_webhook)
        ...
        # Group sessions should be attributed to agent creator (not platform user),
        # consistent with other channel handlers.
        session_user_id = creator_id if is_group_chat else platform_user_id
        ...
        sess = await find_or_create_channel_session(
            ...
            group_name=f"DingTalk Group {conversation_id[:8]}" if is_group_chat else None,
        )
        ...
        reply_text = await _call_agent_llm(
            ...
            history=history, user_id=session_user_id,
        )
        ...
        db.add(ChatMessage(
            agent_id=agent_id, user_id=session_user_id,
