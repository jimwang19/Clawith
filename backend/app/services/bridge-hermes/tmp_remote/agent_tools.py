...
async def _jina_search(arguments: dict) -> str:
    ...
    try:
        ...
        timeout=30,
        proxy="http://100.72.228.29:8888",  # Route jina search through WSL Tailscale proxy
    ) as client:
            resp = await client.get(
...
async def _send_dingtalk_message(
    ...
    from app.api.dingtalk import get_cached_session_webhook
    ...
    try:
        async with async_session() as db:
            ...
            # Prefer group webhook when agent is currently in a DingTalk group context.
            sent_via_group_webhook = False
            group_session = None
            try:
                group_sess_r = await db.execute(
                    select(ChatSession)
                    .where(
                ...
                group_session = group_sess_r.scalar_one_or_none()
                if group_session and group_session.external_conv_id:
                    session_webhook = get_cached_session_webhook(group_session.external_conv_id)
                    if session_webhook:
                        async with httpx.AsyncClient(timeout=10) as client:
                        ...
                        if resp.status_code < 300:
                            sent_via_group_webhook = True
                            logger.info(
                                f"[DingTalk] Group webhook message sent for conv={group_session.external_conv_id}"
                            )
                        else:
                            logger.warning(
                                f"[DingTalk] Group webhook send failed status={resp.status_code}, fallback to OTO"
                            )
            except Exception as ex:
                logger.warning(f"[DingTalk] Group webhook send error, fallback to OTO: {ex}")

            # 3. Send message via DingTalk service (OTO fallback)
            if sent_via_group_webhook:
                result = {"errcode": 0, "channel": "group_webhook"}
            else:
            ...
            if result.get("errcode") == 0:
                ...
                try:
                    ...
                    if sent_via_group_webhook and group_session:
                        sess = group_session
                        msg_user_id = sess.user_id
                    elif platform_user:
                        ...
                        msg_user_id = platform_user.id
                    else:
                        sess = None
                        msg_user_id = None

                    if sess and msg_user_id:
